...
wiz.package 'cypherpunk.backend.db.invoice'

class cypherpunk.backend.db.invoice extends wiz.framework.http.database.mongo.baseArray
	collectionName: 'invoice'
	debug: true
	schema: cypherpunk.backend.db.invoice.schema
	upsert: false
	txidKey: 'txid'

	# DataTable methods
	list: (req, res, invoiceType) => #{{{
		return res.send 400, 'invalid type' if not schemaType = @schema.types[invoiceType]
