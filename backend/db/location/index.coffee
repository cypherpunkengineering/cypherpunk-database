...
wiz.package 'cypherpunk.backend.db.location'

class cypherpunk.backend.db.location extends wiz.framework.http.database.mongo.baseArray
	collectionName: 'location'
	debug: true
	schema: cypherpunk.backend.db.location.schema
	upsert: false
	txidKey: 'txid'

	# DataTable methods
	list: (req, res, locationType) => #{{{
		return res.send 400, 'invalid type' if not schemaType = @schema.types[locationType]
